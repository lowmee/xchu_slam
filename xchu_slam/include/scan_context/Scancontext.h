...
class SCManager {
 public:
  ...
  cv::Mat getLastSCRGB(const Eigen::MatrixXd &sc); // 获取编码的cv图像

 public:
  // hyper parameters ()
  const double LIDAR_HEIGHT =
      2.4; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.
  ...
  const double SEARCH_RATIO =
      0.1; // for fast comparison, no Brute-force, but search 10 % is okay. // but may well work for same-direction-revisits, not for reverse-revisits
  ...
  const double SC_DIST_THRES =
      0.5; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold

  // config
  const int TREE_MAKING_PERIOD_ =
      50; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost

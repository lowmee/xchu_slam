...
cv::Mat SCManager::getLastSCRGB(const Eigen::MatrixXd &sc) {

  cv::Mat image = cv::Mat::zeros(cv::Size(60, 20), CV_8UC3);

  //std::cout << "image size: " << image.rows << ", " << image.cols << std::endl;
  for (int i = 0; i < image.rows; i++) { // height
    for (int j = 0; j < image.cols; j++) { // width
      int value = 0;
      if (i < sc.rows() && j < sc.cols()) {
        value = std::ceil((sc(i, j) * 0.25) * 255);
        if (value < 255 && value > 0) {
          image.at<cv::Vec3b>(i, j)[0] = 180;
          image.at<cv::Vec3b>(i, j)[1] = 80;
          image.at<cv::Vec3b>(i, j)[2] = value;
          continue;
        }
      }

      if (image.at<cv::Vec3b>(i, j)[2] == 0) {
  ...
  cv::Mat out_img;
  cv::resize(image, out_img, cv::Size(0, 0), 10, 10);

  //std::string file_name = std::to_string(ros::Time::now().toSec());
  //cv::imwrite("/home/xchu/workspace2/xchu_wwws/src/xchu_slam/pcd/" + file_name + ".png", out_img);
  return out_img;
}
